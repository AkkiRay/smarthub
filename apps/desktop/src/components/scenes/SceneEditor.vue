...
.swatch {
  ...
  // Selected: двойной INSET ring (тёмный gap + светлая обводка) — даёт ощущение
  // «выбрано» без выхода за пределы свотча. Раньше использовался outline +
  // outline-offset, но тот рендерится СНАРУЖИ кружка и обрезается левым/правым
  // краем модалки, когда свотч стоит крайним в строке.
  &.is-selected {
    box-shadow:
      inset 0 0 0 2px #fff,
