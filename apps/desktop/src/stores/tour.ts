...
const STEPS: TourStep[] = [
  ...
  },
  {
    ...
    title: 'Главная — KPI-дашборд',
    body: 'На главной — четыре counters: устройства в сети, оффлайн, ваши сценарии, статус Алисы. Каждая плитка кликабельна и ведёт на соответствующий раздел.',
    bullets: [
      'В сети — сколько устройств отвечает прямо сейчас',
      'Оффлайн — то что нужно проверить (питание/Wi-Fi)',
      'Сценарии — счётчик локальных + те что у Алисы',
      'Алиса — есть ли подключение к колонке и skill-мост',
    ],
    placement: 'bottom',
    route: '/home',
    tip: 'Кликни KPI «оффлайн» — откроется фильтр «нужно проверить» в /devices.',
  },
  {
    chapter: 'layout',
  ...
  // ---------------- 3 · Устройства ----------------
  {
    ...
    body: 'Хаб опрашивает 25+ протоколов параллельно — Yeelight, Shelly, Hue, Tuya, MQTT, HomeKit, Matter. Multi-interface mDNS: даже если ПК по проводу, а колонка по Wi-Fi — хаб найдёт оба.',
    bullets: [
      'Локальные (Wi-Fi/LAN) — без облака, мгновенно',
      'Облачные (Tuya, Govee, Sber) — нужны API-ключи в Настройках',
      'Кандидаты привязаны к текущей сети — при переезде призраки не остаются',
    ],
    placement: 'right',
  },
  {
  ...
  {
    chapter: 'alice',
    target: 'alice-home-devices',
    title: 'Активный дом + сеть',
    body: 'Если в Yandex-аккаунте несколько домов — выбери активный в селекторе. Хаб запоминает SSID/MAC-роутера и привязывает к нему дом. Подключился к другой сети → автоматический switch household, чужие устройства не появятся.',
    bullets: [
      'Bound к активному дому → cloud-команды разрешены',
      'Чужая сеть → sync блокируется (NETWORK_MISMATCH)',
      'Toggle «Разрешить с любой сети» — отключает gate (для удалённого управления)',
    ],
    placement: 'top',
    route: '/alice?section=station',
    tip: 'Видно текущий SSID и tag «привязана к активному дому» — всегда понятно где ты.',
  },
  {
    chapter: 'alice',
  ...
  },
  {
    ...
    body: 'Краткая шпаргалка:',
    bullets: [
      'Поиск → Сканировать → подключить устройство',
      'Устройства → клик по карточке → управлять',
      'Сценарии → создать → «Доступно через Алису»',
      'Алиса → дорожки «Колонка» и «Skill-мост» независимы',
      'Network-gate защищает от управления чужими устройствами с другой сети',
    ],
