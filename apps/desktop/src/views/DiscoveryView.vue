    ...
    >
      ...
      <div class="discovery__hint-body">
        ...
        <p>
          ...
          <strong>транслируют по локальной сети только после привязки</strong> в приложении «Дом с
          Алисой» (для не-привязанных Wi-Fi называется «smartlife-XXX»). Чтобы Hub их детектил:
        </p>
        <ol>
          <li>Включите устройство в розетку и дождитесь мигания (режим сопряжения).</li>
          <li>
            В приложении <strong>«Дом с Алисой»</strong> добавьте устройство → выберите тип (лампа /
            розетка / выключатель / увлажнитель / …) → введите Wi-Fi-пароль и привяжите к домашней
            сети.
          </li>
          <li>
            Hub автоматически подтянет его через драйвер
            <strong>«Дом с Алисой»</strong> (yandex-iot) — устройство появится во вкладке
            «Устройства» с полным управлением.
          </li>
        </ol>
        <p class="discovery__hint-note">
          Если устройство УЖЕ привязано к чужому Tuya/SmartLife аккаунту — Hub увидит его в LAN, но
          не сможет управлять. Сделайте сброс (обычно 5 быстрых вкл/выкл) и привяжите заново.
        </p>
      </div>
      ...
      >
        ...
        <div class="candidate__body">
          ...
          <p v-if="needsYandexHomeBinding(c) && !isPaired(c)" class="candidate__hint">
            Устройство найдено в локальной сети, но протокол требует ключ из облака. Привяжите его
            через приложение <strong>«Дом с Алисой»</strong> — Hub автоматически подтянет через
            драйвер «yandex-iot» с полным управлением.
          </p>
        </div>
