...
# Локальный профиль для отладки уже лежит в .env.development
pnpm dev
```
...
Все переменные опциональны. Локальный профиль по умолчанию:
[`.env.development`](.env.development).

| Переменная                  | Значение                                          |
| --------------------------- | ------------------------------------------------- |
| `LOG_LEVEL`                 | debug / info / warn / error                       |
| `HUB_DISCOVERY_INTERVAL_MS` | интервал между active-discovery циклами           |
| `HUB_DISCOVERY_TIMEOUT_MS`  | timeout одного scan-а драйвера                    |
| `HUB_POLL_INTERVAL_MS`      | как часто опрашивать online-устройства (0 = выкл) |
| `HUB_ENABLE_MOCK`           | `true`/`false` — включить симулятор               |
| `HUB_OPEN_DEVTOOLS`         | открывать DevTools в dev-режиме                   |
...
├── .env.development                 # ready-to-go профиль для отладки
├── .env.example
└── README.md
```
