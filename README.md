...
Готовые сборки лежат на странице [GitHub Releases](https://github.com/AkkiRay/smarthub/releases/latest):

| ОС          | Файл                                                                                                              |
| ----------- | ----------------------------------------------------------------------------------------------------------------- |
| **Windows** | `SmartHome-Hub-Setup-<ver>.exe` (NSIS-инсталлер) или `SmartHome-Hub-<ver>-portable.exe` (portable, без установки) |
| **macOS**   | `SmartHome-Hub-<ver>-arm64.dmg` (Apple Silicon) или `SmartHome-Hub-<ver>-x64.dmg` (Intel)                         |
| **Linux**   | `SmartHome-Hub-<ver>.AppImage` (universal) или `smarthome-hub_<ver>_amd64.deb`                                    |

После установки приложение само следит за обновлениями (electron-updater +
GitHub Releases): уведомление о новой версии прилетает в раздел
«Настройки → Обновления», там же можно скачать и перезапустить.

> **Windows SmartScreen.** Сборка пока не подписана EV-сертификатом — при
> первом запуске Windows покажет предупреждение «Неизвестный издатель».
> Жмите «Подробнее → Выполнить в любом случае».
> **macOS Gatekeeper.** Без notarization потребуется правый клик по `.app`
> → «Открыть» при первом запуске.
...
# Создайте `.env.development` в корне для override'ов (см. «Конфигурация»).
pnpm dev
```

> Установочные сборки конечному пользователю собираются в CI (`release.yml`)
> и публикуются в GitHub Releases — см. секцию «Скачать» выше.
